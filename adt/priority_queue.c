...
void pq_decrease_key(priority_queue heap, unsigned int position,
                     element_type delta);

void pq_increase_key(priority_queue heap, unsigned int position,
                     element_type delta);
...
int pq_is_full(priority_queue pq) { return pq->max_size == pq->size + 1; }
...
void pq_decrease_key(priority_queue heap, unsigned int pos,
                     element_type delta) {
  if (pos > heap->size) {
    return;
  }
...
void pq_increase_key(priority_queue heap, unsigned int pos,
                     element_type delta) {
  if (pos > heap->size) {
    return;
  }
